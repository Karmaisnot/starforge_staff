  ...
  return (
    <SfFrame>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <div style={{ width: 38, height: 38, borderRadius: 12, background: 'var(--sf-surface-2)',
                           display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {React.cloneElement(Icons.edit, { size: 18 })}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px 18px 100px',
        ...
        <div className="sf-card" style={{ marginTop: 10, padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>Vazifalar</div>
            <span style={{ fontSize: 11, color: 'var(--sf-primary)', fontWeight: 600 }}>
              3 bugun ›
            </span>
          </div>
          {[
            { t: 'May oyi yakuniy hisoboti', pri: 'P1', dl: 'Erta · 18:00', urgent: true, mgmt: true },
            { t: 'Slaydlarni yangilash', pri: 'P2', dl: 'Pen', mgmt: false },
          ].map((x, i, a) => (
            <div key={i} style={{
              display: 'flex', alignItems: 'center', gap: 10,
              padding: '8px 0',
              ...
              <div style={{ width: 14, height: 14, borderRadius: 4, border: '1.5px solid var(--sf-border-strong)' }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12.5, fontWeight: 600 }}>{x.t}</div>
                <div style={{ marginTop: 2, display: 'flex', gap: 6, fontSize: 10, color: 'var(--sf-muted)' }}>
                  {x.mgmt && <span style={{ padding: '1px 5px', borderRadius: 3, background: 'var(--sf-ink)',
                                              color: 'var(--sf-bg)', fontWeight: 700, fontSize: 9 }}>BOSHQ.</span>}
                  <span className="sf-mono" style={{ color: x.urgent ? 'var(--sf-danger)' : 'var(--sf-ink-2)',
                                                       fontWeight: x.urgent ? 700 : 500 }}>{x.dl}</span>
                </div>
              </div>
              <span className="sf-mono" style={{ fontSize: 10, fontWeight: 700,
                                                    color: x.pri === 'P1' ? 'var(--sf-danger)' : 'var(--sf-warn)' }}>{x.pri}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
          {[
            { l: 'Davomat', i: Icons.check, c: 'var(--sf-primary)' },
            { l: 'Karta', i: Icons.brand, c: 'var(--sf-accent)' },
            { l: 'Vazifa', i: Icons.plus, c: 'var(--sf-success)' },
            { l: 'Print', i: Icons.print, c: 'var(--sf-ink-2)' },
          ].map((a, i) => (
            <div key={i} className="sf-card" style={{ padding: '10px 4px', textAlign: 'center' }}>
              <div style={{
                width: 32, height: 32, borderRadius: 10, margin: '0 auto',
                background: 'var(--sf-surface-2)', color: a.c,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>{React.cloneElement(a.i, { size: 16 })}</div>
              <div style={{ marginTop: 4, fontSize: 10.5, fontWeight: 600 }}>{a.l}</div>
            </div>
          ))}
        </div>
        ...
        <button style={{
          ...
          background: 'transparent', border: '1.5px dashed var(--sf-border-strong)',
          borderRadius: 14, color: 'var(--sf-muted)',
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          fontFamily: 'inherit', fontSize: 13, fontWeight: 600, cursor: 'pointer',
        }}>
          {React.cloneElement(Icons.edit, { size: 14 })} Bosh sahifani tahrirlash · widget‘lar
        </button>
      </div>
